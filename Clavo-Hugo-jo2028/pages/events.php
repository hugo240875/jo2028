...
<head>
    ...
    <title>Calendrier des évènements - Jeux Olympiques - Los Angeles 2028</title>
</head>
...
<body>
    ...
    <main>
        ...
        try {
            // Requête pour récupérer le calendrier des épreuves avec le sport et le lieu associé
            // La table EPREUVE contient un id_lieu référencé dans la table LIEU (voir database/test-data-jo.sql)
            $query = "
                SELECT 
                    SPORT.nom_sport, 
                    EPREUVE.nom_epreuve, 
                ...
                ORDER BY EPREUVE.date_epreuve, EPREUVE.heure_epreuve
            ";
            ...
            if ($statement->rowCount() > 0) {
                echo "<table>";
                echo "<tr><th class='color'>Sport</th><th class='color'>Épreuve</th><th class='color'>Date</th><th class='color'>Heure</th><th class='color'>Lieu</th></tr>";
            ...
            error_log("Erreur PDO (events.php) : " . $e->getMessage());
